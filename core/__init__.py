...
from core.data._types import BatchedDataDict
from core.data.augmentation import BaseAugmentor, StemAugmentor
from core.utils import audio as audio_
from core.utils.audio import BaseFader

import torch.optim as optim
try:
    import torch_optimizer as gooptim  # 3rd-party (pip install torch-optimizer)
except Exception:
    gooptim = None
...
# SI-SDR helpers
# ----------------------------
def _bct_to_bt(x: torch.Tensor) -> torch.Tensor:
    """Ensure shape [B, T] by collapsing channels."""
    if x.dim() == 3:  # [B, C, T]
        return x.mean(dim=1)
    if x.dim() == 2:  # [B, T]
        return x
    ...
    raise ValueError(f"Unexpected tensor shape for SI-SDR: {tuple(x.shape)}")
...
def si_sdr(pred: torch.Tensor, target: torch.Tensor, eps: float = 1e-8) -> torch.Tensor:
    ...
    Accepts [B, C, T] or [B, T] (channels are averaged if present).
    Returns [B] (per-example).
    """
    p = _bct_to_bt(pred.float())
    t = _bct_to_bt(target.float())

    # zero-mean
    p = p - p.mean(dim=-1, keepdim=True)
    t = t - t.mean(dim=-1, keepdim=True)

    # projection of p onto t
    alpha = (torch.sum(p * t, dim=-1, keepdim=True)) / (torch.sum(t * t, dim=-1, keepdim=True) + eps)
    s_target = alpha * t
    e_noise = p - s_target
...
def get_optimizer_class(name: str) -> Type[optim.Optimizer]:
    if name == "DeepSpeedCPUAdam":
        return DeepSpeedCPUAdam  # type: ignore[name-defined]

    # Try importing torch_optimizer if available
    try:
        import torch_optimizer as gooptim  # type: ignore
        modules = [optim, gooptim]
    except ImportError:
        modules = [optim]

    # Search through optimizer modules
    for module in modules:
        if name in module.__dict__:
            return module.__dict__[name]
        # Handle lowercase or capitalized variants (e.g., "adam" → "Adam")
        if name.capitalize() in module.__dict__:
            return module.__dict__[name.capitalize()]

    raise NameError(f"Optimizer '{name}' not found in torch.optim or torch_optimizer")
...
class LightningSystem(pl.LightningModule):
    ...
    def update_metrics(
        ...
        # --- SI-SDR logging (once per batch, after metric updates) ---
        try:
            out_audio = output.get("audio", {})
            tgt_audio = batch.get("audio", {}) if isinstance(batch, dict) else batch["audio"]

            # We'll compute SI-SDR for the same stems your metric loop iterates over,
            # reusing the exact matching logic (VOX/BG fallback + mne synthesis).
            si_vals = []
            # determine batch size for logging aggregation
            try:
                bsz = next(iter(out_audio.values())).size(0)
            except Exception:
                bsz = None

            for stem_key in (self.test_metrics if mode == "test" else self.metrics).keys():
                stem = "mne" if stem_key == "mne:+" else stem_key

                # establish whether target exists or can be matched by your fallback rules
                matched = stem in tgt_audio
                if not matched:
                    if stem in self._VOX_STEMS:
                        for bstem in self._VOX_STEMS:
                            if bstem in tgt_audio:
                                tgt_audio[stem] = tgt_audio[bstem]
                                matched = True
                                break
                    elif stem in self._BG_STEMS:
                        for bstem in self._BG_STEMS:
                            if bstem in tgt_audio:
                                ...
                                break
                if not matched:
                    continue  # skip SI-SDR for this stem if no valid target

                # build prediction for this stem (handle "mne" by summing)
                if stem == "mne":
                    # synthesize if missing in output
                    if "mne" not in out_audio:
                        if "music" in out_audio and "effects" in out_audio:
                            pred_sel = out_audio["music"] + out_audio["effects"]
                        else:
                            continue  # can't form mne -> skip
                    else:
                        pred_sel = out_audio["mne"]
                else:
                    if stem not in out_audio:
                        continue  # no prediction for this stem
                    pred_sel = out_audio[stem]

                tgt_sel = tgt_audio[stem]

                # compute SI-SDR and log
                sdr_b = si_sdr(pred_sel, tgt_sel)  # [B]
                sdr = sdr_b.mean()
                self.log(
                    f"{mode}/si_sdr_{stem}",
                    sdr,
            ...
            if si_vals:
                si_avg = torch.stack(si_vals).mean()
                self.log(
                    f"{mode}/si_sdr_avg",
                    si_avg,
                    prog_bar=True,
            ...
            print(f"[warn] SI-SDR logging skipped: {e}")
